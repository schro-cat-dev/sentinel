...
### shared/utils/error-utils.ts [DEAD — 全176行]

`isPiiSafe`, `maskPiiContext`, `safeContext`, `serializeForAudit`, `logFinancialError`, `classifyError`, `getErrorMessage`。セキュリティ修正（/gフラグ除去）は適用済みだがパイプラインからは未使用。
...
| `traceInfo` | log.ts:53 | **normalizeで欠落** | DEAD + BUG |
| `agentBackLog` | log.ts:55 | **normalizeで欠落** | DEAD + BUG |
