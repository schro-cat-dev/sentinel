...
export class LogNormalizer implements ILogNormalizer {
    ...
    normalize(raw: Partial<Log>): Log {
        ...
        return {
            ...
            logicalClock: typeof raw.logicalClock === "number" && Number.isFinite(raw.logicalClock) && raw.logicalClock >= 0
                ? raw.logicalClock
                : (monotoniClock = Math.max(monotoniClock + 1, Date.now())),
            ...
            triggerAgent: typeof raw.triggerAgent === "boolean" ? raw.triggerAgent : false,
            tags: raw.tags ?? [],
            spanId: raw.spanId,
